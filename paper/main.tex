...
\begin{proof}
 ...
 \begin{itemize}
  \item $p \in \Type$のとき, 帰納法の仮定より $\iprt{p}{\rho} \in \iprt{\star}{\xi} = \SN$.
        \[
        \iprt{A}{\rho} = \iprt{\lambda p. q}{\rho} = K (\lambda . \iprt{q}{\rho[0]}) \iprt{p}{\rho} \rightarrow_\beta \lambda . \iprt{q}{\rho[0]} \in \iprt{B}{\xi} = \iprt{\Pi p. r}{\xi} = \iprt{p}{\xi} \rightarrow \iprt{r}{\xi}
        \]
        % $\iprt{A}{\xi} = \iprt{\lambda p. q}{\xi} = \iprt{q}{\xi}$
        % $\iprt{B}{\rho} = \iprt{\Pi p. r}{\rho} = d \iprt{p}{\rho} (\lambda . \iprt{r}{\rho[0]})$
 \end{itemize}
 \end{itemize}
\end{proof}

% \begin{lem}
%  文脈$\Gamma$とそのconstructor valuation $\xi : \mathbb{N} \rightarrow (\SAT)^\star$, object valuation $\rho : \mathbb{N} \rightarrow \Lambda$および$\iprt{\cdot}{\rho} : \GTerm \setminus \{\square\} \rightarrow \Lambda$を考える.
%  このとき, $\rho$に対する依存を分離することができる. すなわち,
%  \[
%  \iprt{p}{\rho} = \lceil p \rceil (\rho)
%  \]
% \end{lem}
