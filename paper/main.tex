...
またいくつかの定理は証明をCoqで行なった.

以下では各章の内容についてまとめる.

章\ref{tremrew}では一般の二項関係について簡単な事実と記号を整理した.

章\ref{lambda}ではCoCの原型となるラムダ計算を紹介し, 合流性を証明した. 合流性の証明はCoCに対してもまったく同様に適用できるが, 簡潔さのためラムダ計算に対して証明を与えた.
おおよそこの章がCoqでのCodeと対応している. 代表的に対応を与えると 系\ref{paraP}とparallelt\_betatP, 定理\ref{CR}とCRが対応している.

章\ref{CoC}ではCoCの型づけ規則を紹介し, 論理的な整合性をうみだす停止性について証明した.
また, 本文章の停止性の証明は \cite{geuvers1994short}によるところが大きい. 具体的には章\ref{CoC}の後半の構成および証明は \cite{geuvers1994short}にのっとったものとなっている.

% 定理\ref{CR}のCoqによる形式化を付録に付ける.

\section{項書き換え系}\label{tremrew}
...
\section{Calculus of Construction}\label{CoC}
