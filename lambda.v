...
Proof.
  elim: (wf_wfr_term t11) t12 t2 => {t11} t11 _ IH.
  case: t11 IH.
  * move=> m _ t12 t2 [][] // n.
    elim: n m t2 t12 => // n IHn m t2 t12.
    rewrite tcSn => [][] c [].
    case: c => []//[]//.
  ...
  * case=> //.
    ...
    - move=> t _ t12 t2 [] x.
      elim: x t12 t2 t => // x IHx t12 t2 t.
      rewrite tcSn => [][] c [].
      case: c => []//[]// c1 c2.
      ...
      case: c => //.
       by move=> ? ? /tcn_betat /betat_abs [].
      by case=> []//[]//[]//[]// ? ? /tcn_betat /id_peel_abs.
    - move=> t t' _ t12 t2 [] x.
      elim: x t t' t12 t2 => // x IHx t t' t12 t2.
      rewrite tcSn => [][] c [].
      case: c => []//[]//[]//; last first.
       move=> ? ? ?.
      ...
      move=> ? /(IHx _ _ _ _).
       ...
       case: t IH => //=.
      rewrite /=.
      rewrite /= !shift_shift !addn0 !shiftnn !orbF.
...
(*      rewrite /= !orbF => tt /tcn_betat. *)
(*      apply. *)
(*      apply beta_betat. *)
(*      rewrite /=. *)
(*       shift_shift shiftnn => /eqP [] <- /tcn_betat /(betat_trans _). *)
(*       apply; apply betatApC => //. *)
(*       by move: a; rewrite /= !orbF; auto. *)
...
(*      case: c => []//[]//. *)
(*       case=> // ?. *)
(*       rewrite /= shift_shift shiftnn => /eqP [] <- /tcn_betat /(betat_trans _). *)
(*       apply; apply betatApC => //. *)
(*       by move: a; rewrite /= !orbF; auto. *)
(*      case=> []//[]//[]//[]//[]//. *)
(*      case: n => // ? ? a. *)
...
(*   move=> ? /=. *)
(*   case=>[][]// n. *)
(*   elim: n t11 t12 t2 => // n IHn t11 t12 t2. *)
(*   rewrite tcnS => [][] c []. *)
(*   case: c => []//[]//[]// c1 c2. *)
(*   case: c1. *)
(*    move=> H /eqP t20. *)
(*    rewrite /= shift_shift addn0 shiftnn in t20. *)
(*    rewrite t20 in H. *)
(*    move/tcn_betat: H. *)
(*    by rewrite betat_id_id. *)
(*   move=> ? tcn' /eqP H. *)
(*   move: H tcn' => <- H. *)
(*   rewrite /= addn0 subn1 /=. *)
(*   apply IHn. *)
...
Lemma subprf_CR t11 t12 c1 :
  ...
  | Abs M =>
    match c1 with
    | App M21 M22 => beta t11 M21 && beta t12 M22 || (t11 == M21) && beta t12 M22 || beta t11 M21 && (t12 == M22) || (shift (subst M 0 (shift t12 1 0 0)) 0 1 0 == c1)
    | _ => shift (subst M 0 (shift t12 1 0 0)) 0 1 0 == c1
    end
  | _ => match c1 with
        | App M21 M22 => beta t11 M21 && beta t12 M22 || (t11 == M21) && beta t12 M22 || beta t11 M21 && (t12 == M22)
        | _ => false
        end
  end -> betat (App t11 t12) c1.
Proof.
elim: t11 c1 t12 => //.
- move=> ? [] // ? ? ?.
  rewrite /= orbF => /andP [] /eqP <- H.
  by apply betatApC; auto.
- move=> t _ c1 t12.
  case t12c1: (shift (subst t 0 (shift t12 1 0 0)) 0 1 0 == c1).
   move/eqP: t12c1 => <- _.
   by apply beta_betat, betaE.
  case: c1 t12c1 => // ? ? _ /andP [] // H1' _.
  apply betatApC.
   case/orP: H1' => [/orP []|] /andP []; auto.
   by move/eqP <-.
  case/orP: H1' => [/orP []|] /andP []; auto.
  by move=> ? /eqP <-.
- move=> t IHt t0 IHt0 c1 t12.
  case: c1 => //= c1 c2.
  case/orP => [/orP []|] /andP [] => [/(IHt _ _) ??|/eqP <- ?|/(IHt _ _) ? /eqP <-];
  by apply betatApC; auto.
Qed.
